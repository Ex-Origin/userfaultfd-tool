...
{
    void *args[PARAMETER_MAX];
    void (*job)(void *, ...);
} thread_parameter;

#ifdef __x86_64__
void *real_run_job(void *p)
{
    thread_parameter *param = (thread_parameter *)p;

    param->job(param->args[0], param->args[1], param->args[2], param->args[3], param->args[4]);
...
void run_job(void (*job)(void *, ...), ...)
{
    ...
    pthread_t tid;
    /*
     * Save parameter into stack.
     * Note: Maybe it will doesn't work very well in some compiler.
     **/
    asm(
        "lea %0, %%rax\n"
        "mov %%rsi, 0(%%rax)\n"
